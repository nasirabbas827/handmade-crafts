...
<head>
  ...
  <title>Online HandCraft Haven</title>
...
// Fetch the site content from DB (assuming id = 1 is the main content)
$sql = "SELECT * FROM site_content WHERE id = 1 LIMIT 1";
$result = mysqli_query($conn, $sql);
$content = mysqli_fetch_assoc($result);
?>

<div class="jumbotron text-center" style="background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('./admin/<?php echo $content['image_url']; ?>'); background-size: cover;">
    <h1><?php echo htmlspecialchars($content['title']); ?></h1>
    <p><?php echo nl2br(htmlspecialchars($content['content'])); ?></p>
    <a href="login.php" class="btn btn-primary btn-lg">Login to Start</a>
</div>
